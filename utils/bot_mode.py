...
async def file_handler(client: Client, message: Message):
    ...
    # Determine the file object (document, video, audio, etc.)
    file = (
    ...
    # Upload file to the TG Drive website
    DRIVE_DATA.new_file(
    ...
    # File ID is stored in `copied_message.id` (which is unique)
    file_id = copied_message.id  # The unique file ID generated by Telegram

    # Create the base URL for the file using the file ID, and assume the file is at the root ('/')
    file_base_url = f"https://jolly-lobster-thunderlinks-43a7df8c.koyeb.app/file?path=/{file_id}"

    # Generate the stream URL by embedding the file URL
    file_stream_url = f"https://jolly-lobster-thunderlinks-43a7df8c.koyeb.app/stream?url={file_base_url}"

    # Send the file upload success message along with the URL
    await message.reply_text(
        f"""✅ File Uploaded Successfully To Your TG Drive Website
                             
**File Name:** {file.file_name}
**File ID:** {file_id}
**File URL:** [Click Here to View the File]({file_stream_url})
"""
    )
